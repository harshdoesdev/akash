...
const vertexShader = /* glsl */ `
  ...
  uniform vec4 uStreamDirs[${STREAMS}]; // xyz travel dir
  uniform float uTime;
  varying vec2 vUv;
  ...
  void main() {
    ...
    vec3 dir = uStreamDirs[int(aStream + 0.5)].xyz;
    float ph = aData.x;
...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    vec3 base = mix(vec3(0.55, 0.72, 0.30), vec3(0.83, 0.76, 0.36), vJitter);
    vec3 col;
    if (uReady > 0.5) {
...
export function createWindLeaves(scene, heightAt, treeColliders, fog) {
  ...
  function respawn(s, dronePos) {
    // Blow off a tree canopy near the drone when one is around.
    ...
    for (let tries = 0; tries < 24; tries++) {
      ...
      if (!c) break;
      if (Math.hypot(c.x - dronePos.x, c.z - dronePos.z) < 240) {
        sx = c.x;
        sz = c.z;
